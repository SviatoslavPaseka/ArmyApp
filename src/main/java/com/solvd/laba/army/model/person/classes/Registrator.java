...
import com.solvd.laba.army.functionalInterfaces.ICalculateSalary;
...
public class Registrator extends Person implements IRegistrator {
	...
	public Registrator(Integer id, String name, Gender gender, LocalDate dob, Boolean haveMedicalExamination,
			Integer numberRegistration) {
	...
	@Override
	public void registerTransport(Transport transport, TypeTransportRegistration typeTransportRegistration) {
	...
	ICalculateSalary<Integer, Double> bonusByRegister = (amountRegister, coefficient) -> {
		Double trueCoeff = 1.0;
		return (trueCoeff == coefficient)?	amountRegister * 12.4 * trueCoeff : 
						(trueCoeff > coefficient)? amountRegister * 13 * (coefficient + trueCoeff * 0.8/10):
							amountRegister * 11 * (coefficient - (coefficient - trueCoeff) * 0.71);
	};
